...
您可以更改其中部分或全部的时长，或者提供您想要的时长（供 `create()` 助手使用）。 此示例显示了所有默认值（以毫秒为单位），但您只需要提供您想要更改或添加的键。
...
您可以通过提供一个自定义的 CSS <code>transition-timing-function</code> 值来改变部分或全部的缓动值，或者提供您自己的缓动值。
