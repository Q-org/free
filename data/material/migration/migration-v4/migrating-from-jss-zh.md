...
请注意，您可以继续使用JSS为组件添加重写（例如`makeStyles`, `withStyles`），即使在迁移到v5之后。 然后，如果在任何时候您想转移到新的样式引擎，您可以逐步重构您的组件。

:::info
如果您正在使用Next.js，并且不确定如何配置SSR以与Emotion和JSS一起工作，可以看一下这个[例子项目](https://github.com/mui/material-ui/tree/master/examples/nextjs-with-typescript-v4-migration)。
:::

本文档回顾了从JSS迁移的所有必要步骤。

虽然您可以使用以下两个选项中的任何一个，但第一个被认为是最好的:
...
:::info
通常情况下，您不会写这样的样式。 但这是我们能用codemod创造的最好的转变。

如果您以后想完善它们，您可以参考下面几节中的例子。
:::
...
:::warning
您应该在一小块文件上运行这个代码模型，然后在继续之前检查变化，因为在某些情况下，您可能需要在转换之后调整代码--这个代码模型不会涵盖所有情况。
:::
...
在某些情况下，您可能想在一个文件中创建多个styled的组件，而不是增加CSS的特殊性。
...
如果您[使用`styled-components`作为底层样式引擎来代替`@emotion`](/material-ui/guides/interoperability/#styled-components)，那么这个API将无法工作。
:::

该API类似于JSS的`makeStyles`，但在hood之下，它使用`@emotion/react`。 它还具有比v4的`makeStyles`更好的TypeScript支持。

为了使用它，您需要把它添加到您的项目的依赖项中:
...
如果您使用`$`语法和`clsx`来组合多个CSS类，那么转变会是这样的。
...
在运行该代码后，在您的代码中搜索 "TODO jss-to-tss-react codemod"，以找到该代码不能可靠地处理的情况。
...
:::error
您应该放弃[`clsx`](https://www.npmjs.com/package/clsx)而选择[`cx`](https://emotion.sh/docs/@emotion/css#cx)。
...
为了确保您的类名总是包括您的组件的实际名称，您可以将`名称`作为一个隐式命名的键来提供(`name: { App }`).
...
如果您解构了一个以上的项目，您可能会遇到[类似这样](https://user-images.githubusercontent.com/6702424/148657837-eae48942-fb86-4516-abe4-5dc10f44f0be.png)的eslint警告。
...
默认情况下，TSS只能够提供主题。 如果您想提供props和`ownerState`，[请参考这个文档](https://docs.tss-react.dev/mui-theme-styleoverrides)。

:::warning
tss-react_不是_由MUI维护的。

如果您对如何设置SSR（Next.js）有任何疑问，或者您想知道如何定制`主题`对象，请参考tss-react文档-特别是[MUI集成部分](https://github.com/garronej/tss-react#mui-integration)。

您也可以为任何错误或功能请求[提交一个问题](https://github.com/garronej/tss-react/issues/new)，如果您需要帮助，可以[开始讨论](https://github.com/garronej/tss-react/discussions)。
:::
...
一旦您迁移了所有的样式，通过卸载软件包来移除不必要的`@mui/styles`。
...
:::warning
`@emotion/styled` is a peer dependency of `@mui/material`. 您必须在您的依赖关系中保留它，即使您从未明确使用它。
:::
